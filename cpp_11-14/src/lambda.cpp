...
template <typename T = int32_t> class Base {
private:
  T _val;
...
public:
  ...
  virtual int32_t GetMultipliedVal(const int32_t &mult) const override {
    return (_dVal * mult);
  }
};
...
auto mult = [](int32_t a, int32_t b) -> int32_t { return (a * b); };
...
int32_t main(const int32_t argc, const char **argv) {
  ...
  {
    {
      ...
      sort(v.begin(), v.end(),
           [](const int32_t &a, const int32_t &b) -> bool { return a > b; });
      ...
      vector<int32_t>::iterator iter =
          find_if(v.begin(), v.end(), [](int32_t a) { return a < 5; });
    ...
    {
      shared_ptr<Base<>> sb(new Base<>(1), [](Base<> *ptr) {
        cout << "Deletion skipped" << endl;
      });
      ...
      sb->setLambda([sb](const int32_t &a) {
        cout << (sb->GetMultipliedVal(a) * 2) << endl;
      });
      sb->lambdaCall(5);
  ...
  {
    shared_ptr<Base<>> sb(
        new Base<>(1), [](Base<> *ptr) { cout << "Deletion skipped" << endl; });
    unique_ptr<Base<>, decltype(delBase)> ub(new Base<>(2), delBase);
    cout << endl;
    unique_ptr<Base<>, void (*)(Base<> *)> ub2(new Base<>(3), [](Base<> *b) {
      cout << "inline lambda delete called\n";
      delete b;
    });
    unique_ptr<Base<>, std::function<void(Base<> *)>> ub3(
        new Base<>(4), [](Base<> *b) {
