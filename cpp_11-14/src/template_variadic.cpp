...
template <typename T>
void Basic::PrT(const T& val) { cout << val << endl; }

template <typename T = int32_t, size_t NUM = 1>
class Base
{
  private:
    T _vals[NUM];
...
bool CompareT(const T& a, const U& b) { return a == b; }

template<typename T, typename U, typename... Args>
bool CompareT(const T& a, const U& b, Args... args) { return ((a == b) && CompareT(args...)); }

template<typename T = char>
void PrintCharT(const T* a) { cout << a; }
...
typename std::enable_if<
  k == 0, typename VarStructT<0, tuple<Ts...>>::type&>::type
std::get(tuple<Ts...>& t)
{
  return t.tail;
}
...
typename std::enable_if<
  k != 0, typename VarStructT<k, tuple<T, Ts...>>::type&>::type
...
template <typename T, typename... Args> class VarT
{
  ...
  public:
    VarT(const T& val, const Args&... args) : _vars{std::move(args)...}, _commonVal(val) { }
    ~VarT() { }

    void SetCommonVal(const T& val) { _commonVal = val; }
    T& GetCommonVal() { return _commonVal; }

    template <size_t k> auto GetVal() -> decltype(std::get<k>(_vars))& { return std::get<k>(_vars); }
...
template <typename... Args> class VarArrT
{
  ...
  public:
    ...
    template <size_t k> auto getArr() -> decltype(std::get<k>(_vars))& { return std::get<k>(_vars); }
};
...
template <typename T, typename... Args> class Var2T
{
  private:
    ...
    typename std::enable_if<std::is_pointer<T>::value, void>::type checkType() { }
		...
		template <size_t k>	typename std::enable_if<k == 0, void>::type Pr() { printVar<k>(); }
...
template <size_t k>
void Var2T<T, Args...>::printVar() { cout << "[" << k << "] " << std::get<k>(_vars) << endl; }
...
int32_t main(const int32_t argc, const char** argv) {
  ...
  {
    {
      VarT<const char*, double, uint64_t, const char*> var("common elem", 12.2, 42, "big");
    ...
    cout << endl;
    {
      VarArrT<const char*, int32_t> var_arr({"Element 00", "Element 01"}, {10, 11});
    ...
    cout << endl;
    {
      Var2T<const char*, double, uint64_t, const char*> var("1st elem", 1.1, 2, "last elem");
